...
- **Integración de Iconos Animados en Menús de Navegación**: Reemplazados todos los iconos vectoriales estáticos de la barra lateral de navegación (`Layout.js`) con componentes de Lucide React y `AnimatedIcon`. Ahora cada elemento de menú (*Dashboard, Alertas, Calendario, Clientes, Ventas, Consentimientos, Cabinas, Empleados, Comunicaciones, Comercial CRM, Inventario, Contabilidad, Configuración, etc.*) micro-anima su icono con efectos reactivos (`bounce`, `rotate`, `pulse`, `shake`) al pasar el cursor sobre ellos.
