...
const Cliente = {
    ...
    const result = await pool.query(
      `INSERT INTO clientes
        (nombre, apellidos, segundo_apellido, email, telefono, fecha_nacimiento, notas,
         ...
         tutor_legal_nombre, tutor_legal_dni, tutor_legal_telefono,
         info_medica, acepta_comunicaciones, acepta_notificaciones_sistema, acepta_redes, como_nos_conocio,
         sexo, instagram, facebook, tiktok, twitter, estudio_id)
  ...
  async buscarTodos(estudio_id) {
    const result = await pool.query('SELECT * FROM clientes WHERE estudio_id=$1 ORDER BY nombre', [estudio_id]);
    return result.rows;
  },

  async buscarPorId(id, estudio_id) {
    const result = await pool.query('SELECT * FROM clientes WHERE id=$1 AND estudio_id=$2', [id, estudio_id]);
    return result.rows[0];
  },

  async actualizar(id, data, estudio_id) {
    const {
      nombre, apellidos, segundo_apellido, email, telefono, fecha_nacimiento, notas,
      dni, pais, provincia, localidad, direccion, codigo_postal,
      conflictivo, flexible, habla_ingles, es_cliente_estudio, cliente_pruebas,
    ...
    const result = await pool.query(
      `UPDATE clientes SET
       ...
       WHERE id=$32 AND estudio_id=$33 RETURNING *`,
      [nombre, apellidos, segundo_apellido, email, telefono, fecha_nacimiento, notas,
       dni, pais, provincia, localidad, direccion, codigo_postal,
       conflictivo, flexible, habla_ingles, es_cliente_estudio, cliente_pruebas,
       tutor_legal_nombre, tutor_legal_dni, tutor_legal_telefono,
       info_medica, acepta_comunicaciones, acepta_notificaciones_sistema, acepta_redes, como_nos_conocio,
       sexo, instagram, facebook, tiktok, twitter, id, estudio_id]
    );
    return result.rows[0];
  },
  ...
  async incrementarNoShows(id) {
    const result = await pool.query(
      'UPDATE clientes SET no_shows = no_shows + 1 WHERE id=$1 RETURNING *', [id]
    );
    return result.rows[0];
  },

  async buscarPorNombre(texto, estudio_id) {
    const result = await pool.query(
      `SELECT * FROM clientes WHERE estudio_id=$1 AND (nombre ILIKE $2 OR apellidos ILIKE $2 OR email ILIKE $2 OR telefono ILIKE $2) ORDER BY nombre`,
      [estudio_id, `%${texto}%`]
    );
    return result.rows;
  },

  async buscarConFiltros({ conflictivo, flexible } = {}, estudio_id) {
    const conditions = [`estudio_id = $1`];
    const values = [estudio_id];
    if (conflictivo !== undefined) { conditions.push(`conflictivo = $${values.length + 1}`); values.push(conflictivo === 'true' || conflictivo === true); }
    if (flexible !== undefined) { conditions.push(`flexible = $${values.length + 1}`); values.push(flexible === 'true' || flexible === true); }
    const result = await pool.query(`SELECT * FROM clientes WHERE ${conditions.join(' AND ')} ORDER BY nombre`, values);
    return result.rows;
  },
};
