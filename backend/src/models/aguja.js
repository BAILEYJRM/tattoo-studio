...
const Aguja = {
  crear: async (d, estudio_id) => {
    const r = await pool.query(
      `INSERT INTO agujas (marca, modelo, tipo, numero_lote, fecha_caducidad, fecha_fabricacion, producto_id, estudio_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [d.marca||null, d.modelo||null, d.tipo||null, d.numero_lote||null,
       d.fecha_caducidad||null, d.fecha_fabricacion||null, d.producto_id||null, estudio_id]
    );
    return r.rows[0];
  },

  buscarTodas: async ({ buscar, marca } = {}, estudio_id) => {
    const conds = ['a.activa = true', 'a.estudio_id = $1'];
  ...
  buscarPorId: async (id, estudio_id) => {
    const r = await pool.query('SELECT * FROM agujas WHERE id = $1 AND estudio_id = $2', [id, estudio_id]);
  ...
  actualizar: async (id, d, estudio_id) => {
    const r = await pool.query(
      `UPDATE agujas SET marca=$1, modelo=$2, tipo=$3, numero_lote=$4,
       fecha_caducidad=$5, fecha_fabricacion=$6, producto_id=$7 WHERE id=$8 AND estudio_id=$9 RETURNING *`,
      [d.marca||null, d.modelo||null, d.tipo||null, d.numero_lote||null,
       d.fecha_caducidad||null, d.fecha_fabricacion||null, d.producto_id||null, id, estudio_id]
    );
    return r.rows[0];
  },

  desactivar: async (id, estudio_id) => {
    const r = await pool.query('UPDATE agujas SET activa = false WHERE id = $1 AND estudio_id = $2 RETURNING *', [id, estudio_id]);
    return r.rows[0];
  },
