...
const getMovimientos = async (req, res) => {
  try {
    ...
    const movimientos = producto_id
      ? await MovimientoStock.buscarPorProducto(producto_id, req.usuario.estudio_id)
      : await MovimientoStock.buscarTodos(req.usuario.estudio_id);
...
const registrarMovimiento = async (req, res) => {
  try {
    ...
    await Producto.actualizarStock(producto_id, delta, req.usuario.estudio_id);
    const movimiento = await MovimientoStock.crear({
      producto_id, tipo, cantidad: Math.abs(Number(cantidad)), motivo, notas, empleado_id, estudio_id: req.usuario.estudio_id
