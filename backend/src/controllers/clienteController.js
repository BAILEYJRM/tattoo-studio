...
const deleteCliente = async (req, res) => {
  try {
    const { id } = req.params;
    // Realizamos borrado en cascada manual o confiamos en las FK (si existieran ON DELETE CASCADE)
    // Asumiremos que limpiamos todo lo relacionado si no hay cascade.
    // Usaremos una transacción para borrar todo el rastro.
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
...
const bulkDeleteClientes = async (req, res) => {
  const { ids } = req.body;
...
module.exports = { getClientes, getCliente, crearCliente, actualizarCliente, getHistorialCliente, getDuplicados, fusionarClientes, deleteCliente, bulkDeleteClientes };
