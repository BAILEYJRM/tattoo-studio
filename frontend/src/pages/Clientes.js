  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">{t('clientes_titulo')}</h1>
        <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" value={buscar} onChange={(e) => setBuscar(e.target.value)}
            placeholder={t('buscar_cliente_placeholder')}
        ...
        <div className="flex gap-2">
          {[['', t('todos')], ['conflictivo', t('conflictivos')], ['flexible', t('flexibles')]].map(([val, label]) => (
      ...
      <div className="bg-gray-900 rounded-xl overflow-hidden border border-gray-700/60 shadow-sm">
        {loading ? (
          <div className="p-8 text-center text-gray-500 text-sm">{t('cargando')}</div>
        ) : clientesFiltrados.length === 0 ? (
          <div className="p-8 text-center text-gray-500 text-sm">
            {buscar || filtroFlag ? t('sin_resultados') : t('no_hay_clientes')}
          </div>
        ) : (
