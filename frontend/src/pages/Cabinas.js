...
// ─── Tarjeta de cabina ────────────────────────────────────────────────────────
function TarjetaCabina({ cabina, onCambiarEstado, onRegistrarLimpieza, onVerHistorial, onVerIncidencias, onDelete, isSelected, onToggleSelect }) {
    ...
    <div className={`bg-gray-900 rounded-2xl p-5 flex flex-col gap-4 border-2 transition-colors ${
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-3 min-w-0">
          <input type="checkbox"
            checked={isSelected}
            onChange={() => onToggleSelect(cabina.id)}
            className="mt-1 rounded border-gray-600 bg-gray-800 text-indigo-500 focus:ring-indigo-500" />
          <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button
          onClick={() => onDelete(cabina)}
          className="col-span-2 py-2 rounded-lg text-xs font-semibold bg-red-600/10 hover:bg-red-600/20 text-red-400 transition-colors flex items-center justify-center gap-1"
        >
...
export default function Cabinas() {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === cabinas.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(cabinas.map(c => c.id));
    }
  };
  ...
  const handleDelete = async (c) => {
    if (!window.confirm(`¿Seguro que deseas eliminar la cabina ${c.nombre}?`)) return;
    try {
      await deleteCabina(c.id);
      cargarCabinas();
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`¿Eliminar ${selectedIds.length} cabinas?`)) return;
    try {
      await bulkDeleteCabinas(selectedIds);
      cargarCabinas();
      setSelectedIds([]);
    } catch (err) {
      alert(err.response?.data?.error || 'Error al eliminar');
        ...
        <div className="space-y-4">
          {cabinas.length > 0 && (
            <div className="flex items-center gap-2 px-1">
              <input type="checkbox" id="selectAllCabinas"
                checked={selectedIds.length === cabinas.length}
                onChange={toggleSelectAll}
                className="rounded border-gray-600 bg-gray-800 text-indigo-500 focus:ring-indigo-500" />
              <label htmlFor="selectAllCabinas" className="text-sm text-gray-400 cursor-pointer">Seleccionar todas</label>
            </div>
          )}
